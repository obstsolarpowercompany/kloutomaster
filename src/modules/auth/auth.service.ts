...
import { generateAccountNumber } from '../wallet/helpers/wallet.helper';
...
export default class AuthenticationService {
  ...
  async createNewUser(createUserDto: CreateUserDTO) {
    ...
    try {
      ...
      const userExists = await this.userService.getUserByEmailTrans(
        createUserDto.email,
        queryRunner.manager,
      );
  ...
  async confirmLoginByOtp(confirmOtpDto: ConfirmOtpDto, res: any) {
    return await this.dataSource.transaction(async (manager) => {
      const user = await this.userService.verifyOtpForAction(
        confirmOtpDto.email,
        confirmOtpDto.otp,
        manager,
      );
  ...
  async confirmEmailByOtp(confirmOtpDto: ConfirmOtpDto, res: any) {
    return await this.dataSource.transaction(async (manager) => {
      const updatedUser = await this.userService.verifyOtpForAction(
        confirmOtpDto.email,
        confirmOtpDto.otp,
        manager,
      );
  ...
  async resendOtp(resendOTPDto: ResendOTPDto, res: any) {
    return await this.entityManager.transaction(async (manager) => {
      ...
      const lastOtp = await this.userService.getLastOtpByEmail(
        email,
        this.entityManager,
      );
