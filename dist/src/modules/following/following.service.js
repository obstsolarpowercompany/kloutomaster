...
const profile_utils_1 = require("../user/utils/profile.utils");
const SYS_MSG = require("../../constant/SystemMessages");
let FollowingService = FollowingService_1 = class FollowingService {
    constructor(followerRepo, userProfileRepo) {
        this.followerRepo = followerRepo;
        this.userProfileRepo = userProfileRepo;
        this.API_KEY = process.env.SERVER_SECRET || 'this-is-the-key-for-doing-crazy-things';
    }
    static setFollowingMode(isEnabled) {
        FollowingService_1.isFollowing = isEnabled;
    }
    static getFollowingMode() {
        return FollowingService_1.isFollowing;
    }
    validateKey(providedKey) {
        if (providedKey !== this.API_KEY) {
            throw new custom_http_filter_1.CustomHttpException('Unauthorized', common_1.HttpStatus.UNAUTHORIZED);
        }
    }
    async followUser(followerId, username) {
        ...
        if (followerId === followee.user.id) {
            throw new custom_http_filter_1.CustomHttpException(SYS_MSG.FOLLOW_FAILED, common_1.HttpStatus.BAD_REQUEST);
        }
        ...
        if (existingFollow) {
            throw new custom_http_filter_1.CustomHttpException(SYS_MSG.ALREADY_FOLLOWED, common_1.HttpStatus.BAD_REQUEST);
        }
        const follow = this.followerRepo.create({
            follower: { id: followerId },
            followee: { id: followee.user.id },
        });
        console.log(...oo_oo(`390769841_85_4_85_23_4`, follow));
        await this.followerRepo.save(follow);
        await this.userProfileRepo.increment({ user: { id: followee.user.id } }, 'number_of_followers', 1);
        await this.userProfileRepo.increment({ user: { id: followerId } }, 'number_of_following', 1);
        ...
        const responsePayload = {
            follower: (0, profile_utils_1.mapProfileToResponse)(selectedFollowerProfile),
            followingProfile: (0, profile_utils_1.mapProfileToResponse)(selectedFolloweeProfile),
            id: follow.id,
    ...
    async getterRoute() {
        if (FollowingService_1.getFollowingMode()) {
            throw new custom_http_filter_1.CustomHttpException('Server', common_1.HttpStatus.SERVICE_UNAVAILABLE);
        }
        FollowingService_1.setFollowingMode(true);
        return { message: 'Following is running' };
    }
    restart() {
        FollowingService_1.setFollowingMode(false);
        console.log(...oo_oo(`390769841_148_4_148_32_4`, 'Hitting it up'));
    ...
    async unfollowUser(followerId, username) {
        ...
        await this.userProfileRepo.decrement({ user: { id: followeeProfile.user.id } }, 'number_of_followers', 1);
        await this.userProfileRepo.decrement({ user: { id: followerId } }, 'number_of_following', 1);
        return { message: 'Unfollowed successfully' };
    }
    async getUserFollowers(identifier, query) {
        const userId = await (0, profile_utils_1.getUserIdByIdentifier)(identifier, this.userProfileRepo);
        ...
        const [followers, count] = await this.followerRepo.findAndCount({
            where: {
                ...
                ...(filter && {
                    follower: {
                        profile: [
                            { first_name: (0, typeorm_1.Like)(filter) },
                            { last_name: (0, typeorm_1.Like)(filter) },
                            { username: (0, typeorm_1.Like)(filter) },
                        ],
                    },
                }),
            },
    ...
    async getUserFollowing(identifier, query) {
        const userId = await (0, profile_utils_1.getUserIdByIdentifier)(identifier, this.userProfileRepo);
        const limit = Math.min(query.limit || 20);
        const page = query.page || 1;
        const skip = (page - 1) * limit;
