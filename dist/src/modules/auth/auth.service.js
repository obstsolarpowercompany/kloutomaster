...
const wallet_helper_1 = require("../wallet/helpers/wallet.helper");
const typeorm_2 = require("@nestjs/typeorm");
const wallet_entity_1 = require("../wallet/entities/wallet.entity");
let AuthenticationService = class AuthenticationService {
    constructor(userService, jwtService, configService, dataSource, entityManager, walletRepo) {
    ...
    async createNewUser(createUserDto) {
        ...
        try {
            ...
            if (userExists) {
                throw new custom_http_filter_1.CustomHttpException(SYS_MSG.USER_ACCOUNT_EXIST, common_1.HttpStatus.BAD_REQUEST);
            }
            const newUser = await this.userService.createUser(createUserDto, queryRunner.manager);
            const accountNumber = await (0, wallet_helper_1.generateAccountNumber)(this.walletRepo);
    ...
    async login(loginDto, res) {
        return await this.dataSource.transaction(async (manager) => {
            ...
            if (!user) {
                throw new custom_http_filter_1.CustomHttpException(SYS_MSG.INVALID_CREDENTIALS, common_1.HttpStatus.UNAUTHORIZED);
            }
            return {
    ...
    async confirmLoginByOtp(confirmOtpDto, res) {
        return await this.dataSource.transaction(async (manager) => {
    ...
    async refreshAccessToken(refreshToken, res) {
        if (!refreshToken) {
            throw new custom_http_filter_1.CustomHttpException('Refresh token missing', common_1.HttpStatus.BAD_REQUEST);
        }
        const storedToken = await this.entityManager.findOne(refreshToken_entity_1.RefreshToken, {
            where: { token: refreshToken },
        });
        if (!storedToken || storedToken.expires < new Date()) {
            throw new custom_http_filter_1.CustomHttpException('Invalid refresh token', common_1.HttpStatus.UNAUTHORIZED);
        }
    ...
    async confirmEmailByOtp(confirmOtpDto, res) {
        return await this.dataSource.transaction(async (manager) => {
    ...
    async resendOtp(resendOTPDto, res) {
        return await this.entityManager.transaction(async (manager) => {
            ...
            if (!user) {
                throw new custom_http_filter_1.CustomHttpException(SYS_MSG.USER_NOT_FOUND, common_1.HttpStatus.NOT_FOUND);
            }
    ...
    async setCookie(token, res) {
        const cookieExpiry = this.configService.get('auth.cookieExpiry');
    ...
    async generateRefreshToken(user) {
        ...
        console.log(...oo_oo(`334045143_297_4_297_58_4`, 'Refresh tojen expiry date: ', expiryDate));
        const refreshToken = await this.jwtService.signAsync(payload, {
            secret: secret,
            expiresIn: tokenExpiryTime + 'd',
        });
        await this.entityManager.save(refreshToken_entity_1.RefreshToken, {
            userId: user.id,
            token: refreshToken,
            expires: expiryDate,
        });
    ...
    async setAuthCookies(accessToken, refreshToken, res) {
        const cookieExpiry = this.configService.get('auth.cookieExpiry');
        ...
        res.cookie('refresh_token', refreshToken, {
            httpOnly: true,
            secure: process.env.NODE_ENV === 'production',
            maxAge: this.configService.get('auth.refreshCookieExpiry'),
        });
    }
};
AuthenticationService = __decorate([
    (0, common_1.Injectable)(),
    __param(5, (0, typeorm_2.InjectRepository)(wallet_entity_1.Wallet)),
